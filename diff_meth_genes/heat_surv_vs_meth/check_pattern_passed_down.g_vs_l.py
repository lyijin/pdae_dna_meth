...
"""
> check_pattern_passed_down.g_vs_l.py <
...
In numbers--if overall inheritance of S7-AD --> L1-AD has a tau of 0.61, if 
we subselected methylated positions in those interesting genes, would these
positions produce a tau > 0.61 (where tau = 1 means perfect concordance)?
"""
import numpy as np
import pandas as pd
import scipy.stats
...
meth_data = meth_data.filter(regex='^[E|L|S].-AD').drop(['S5-AD', 'S6-AD'], 1)
...
with open('check_pattern_passed_down.g_vs_l.tsv', 'w') as f:    
    # print header
    ...
    tau2 = []
    tau3 = []
    for m in ['E8-AD', 'S7-AD', 'S8-AD']:
        for n in range(1, 7):
            temp1 = meth_data[[m, f'L{n}-AD']]
            temp2 = all_genes_data[[m, f'L{n}-AD']]
            temp3 = high_corr_genes_data[[m, f'L{n}-AD']]
    ...
    print ('Paired t-test, all genes vs. genes with strong corr',
           scipy.stats.ttest_rel(tau2, tau3)[1], sep='\t', file=f)
