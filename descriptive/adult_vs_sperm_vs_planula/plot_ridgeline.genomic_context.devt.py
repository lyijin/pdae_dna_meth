...
combined_data = pd.DataFrame({'meth_pct': [], 'genomic_feature': [],
                              'devt': []})
for n, devt in enumerate(['Adult', 'Sperm', 'Egg', 'Larval']):
    data = pd.read_table(
        '../../merged_final_covs/all-{}.filt.annot.merged.cov'.format(devt),
        header=None,
        usecols=[3, 6, 10],
...
g = sns.FacetGrid(combined_data, row='devt', col='genomic_feature', hue='devt',
                  height=1.5, aspect=2.5, palette='Blues_d')
...
g.fig.subplots_adjust(wspace=0.1, hspace=-.8)
...
output_filename = 'ridgeline.genomic_context.devt.pdf'
